              ...
              <div style="text-align: right">
                ...
                <el-tooltip placement="top" content="比赛已结束但未开放，无法提交" :disabled="new Date() < contest.endTime || contest.open">
                  <div style="display:inline-block; margin-top: 20px">
                    <el-button type="danger" size="medium" plain @click="submitCode" :disabled="(new Date() >= contest.endTime && !contest.open) || codeSubmitLoading">
  ...
  methods: {
    update: function () {
      ...
      axios.get("/contests/" + this.id + "/problems/" + this.problemNumber)
          .then((response) => {
            ...
            axios.get("/contests/" + this.id + "/problems/" + this.problemNumber + "/records/recent", {
              params: {
              ...
              if (this.records.length > 0) {
                ...
                axios.get("/contests/" + this.id + "/records/" + this.records[0].id + "/code", {
                  params: {
    ...
    editProblem: function () {
      let routeUrl = this.$router.resolve({
        path: "/problem/" + this.problemDetail.id + "/edit",
      });
    ...
    submitCode: function () {
      ...
      this.loading = true
      axios.post("/contests/" + this.id + "/records", {
        "problemId": this.id,
